...
public abstract class AbstractAction implements Command {
    ...
    protected void popupShouldSave(ActionEvent e) {
        ...
        if (GuiPackage.getInstance().getTestPlanFile() == null) {
            if (JOptionPane.showConfirmDialog(GuiPackage.getInstance().getDialogParent(),
            ...
            if (f.exists()) {
                switch (actionOnFile) {
                    ...
                    case ASK:
                        ...
                        int response = JOptionPane.showOptionDialog(GuiPackage.getInstance().getDialogParent(),
