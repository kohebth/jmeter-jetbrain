    ...
    public void doAction(ActionEvent e) {
        try {
            ...
            if (updateFile != null) {
                ...
                int retVal = jFileChooser.showSaveDialog(GuiPackage.getInstance().getDialogParent());
                if (retVal == JFileChooser.APPROVE_OPTION) {
                    File outputFile = jFileChooser.getSelectedFile();
                    if (outputFile.exists()) {
                        int response = JOptionPane.showConfirmDialog(GuiPackage.getInstance().getDialogParent(),
