...
public class MainFrame extends JFrame implements TestStateListener, Remoteable, DropTargetListener, Clearable, ActionListener {
    ...
    /** Whether this frame only acts as JMeter's hidden GUI controller. */
    private boolean embeddedMode;

    /** The native tree/form surface exported to an embedding host. */
    ...
    public MainFrame(TreeModel treeModel, JMeterTreeListener treeListener) {
        this(treeModel, treeListener, false);
    }

    /**
     * Create a JMeter GUI controller for use inside another Swing application.
     * The frame and its menu/toolbar state are retained for native JMeter
     * actions, while only the tree/form split is exposed to the host.
     *
     * @param treeModel the model for the test tree
     * @param treeListener the listener for the test tree
     * @param embeddedMode true when the native editor is hosted elsewhere
     */
    public MainFrame(TreeModel treeModel, JMeterTreeListener treeListener, boolean embeddedMode) {
    ...
    private void init() { // WARNING: called from ctor so must not be overridden (i.e. must be private or final)
        ...
        Component toolBarComponent = createToolBar();
        if (!embeddedMode) {
            all.add(toolBarComponent, BorderLayout.NORTH);
        }
    ...
    /**
     * Return whether this frame is serving as an embedded action controller.
     *
     * @return true when JetBrains owns file navigation and persistence
     */
    ...
    public void closeEmbedded() {
        ...
        if (guiPackage != null) {
            guiPackage.getLogEventBus().unregisterEventListener(logPanel);
            guiPackage.getLogEventBus().unregisterEventListener(errorsAndFatalsCounterLogTarget);
