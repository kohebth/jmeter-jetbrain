    ...
    protected AbstractVisualizer() {
        ...
        saveConfigButton.addActionListener(e -> {
            SavePropertyDialog d = new SavePropertyDialog(
                    JOptionPane.getFrameForComponent(GuiPackage.getInstance().getDialogParent()),
                    JMeterUtils.getResString("sample_result_save_configuration"), // $NON-NLS-1$
                    true, collector.getSaveConfig());
            d.pack();
            ComponentUtil.centerComponentInComponent(GuiPackage.getInstance().getDialogParent(), d);
            d.setVisible(true);
        });
