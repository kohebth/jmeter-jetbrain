...
public final class GuiPackage implements LocaleChangeListener, HistoryListener {
    ...
    /** Visible parent supplied by an embedding host for native dialogs. */
    ...
    /** Unregister listeners installed by {@link #registerAsListener()}. */
    public void unregisterAsListener() {
        if (UndoHistory.isEnabled()) {
            this.undoHistory.unregisterHistoryListener(this);
        }
    ...
    /**
     * Clear the singleton owned by an embedding host.
     *
     * @param expected instance expected to own the singleton
     */
    public static void disposeInstance(GuiPackage expected) {
        if (GuiPackage.guiPack == expected) {
            GuiPackage.guiPack = null;
        }
    }
    ...
    */
    ...
    /**
     * Set the visible component that should own native JMeter dialogs.
     *
     * @param parent visible editor component, or null to use the main frame
     */
    ...
    /**
     * Return the best visible parent for a native JMeter dialog.
     *
     * @return embedding parent when available, otherwise JMeter's frame
     */
    public Component getDialogParent() {
        return dialogParent != null ? dialogParent : mainFrame;
    }
    ...
    public static void showMessage(final String message, final String title, final int type){
        ...
        SwingUtilities.invokeLater(() -> JOptionPane.showMessageDialog(guiPack.getDialogParent(), message, title, type));
